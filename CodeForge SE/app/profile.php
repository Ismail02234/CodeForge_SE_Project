...
// Older repository results may not include profile counters. Only query them when needed.
if (!array_key_exists('solved_count', $user) || !array_key_exists('submission_count', $user)) {
    $statsStmt = $pdo->prepare(
        "SELECT COUNT(DISTINCT CASE WHEN verdict = 'AC' THEN problem_id END) AS solved_count,
                COUNT(*) AS submission_count
         FROM submissions
         WHERE user_id = :uid"
    );
    $statsStmt->execute(['uid' => $id]);
    $stats = $statsStmt->fetch() ?: [];
    $user['solved_count'] = (int) ($stats['solved_count'] ?? 0);
    $user['submission_count'] = (int) ($stats['submission_count'] ?? 0);
}
...
<section class="hero">
    <div class="hero-grid">
        ...
        <div class="grid grid-2">
            ...
            <div class="card stat"><div class="label">Solved</div><div class="value green"><?= (int) ($user['solved_count'] ?? 0) ?></div></div>
            <div class="card stat"><div class="label">Submissions</div><div class="value"><?= (int) ($user['submission_count'] ?? 0) ?></div></div>
        </div>
    </div>
</section>
