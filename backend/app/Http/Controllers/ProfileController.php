...
class ProfileController extends Controller
{
    public function show(
        ...
        $stats = DB::table('submissions')
            ...
            ->selectRaw(
                "COUNT(*) submissions,
                 ...
                 COUNT(
                    DISTINCT CASE
                        WHEN verdict='AC' THEN problem_id
                    END
                 ) solved"
            )
            ->first();
        ...
        return [
            'user' => $user,
            'stats' => $stats,
            'performance_profile' => $performanceProfile->calculate($id),
            'gamification' => $gamification->forUser($id),
            'recent_submissions' => DB::table('submissions as s')
